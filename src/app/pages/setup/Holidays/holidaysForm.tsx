...
const HolidaysForm = () => {
  ...
  const {mutate: newHolidayEntry} = useMutation((values)=>axios.post(`${Api_Endpoint}/holidays`,values), {
    onSuccess:async (response,values:any)=>{
      message.success("Record submitted successfully")
      var trailData = {
        userId: currentUser?.id,
        action: `User has added a holiday in the system.`,
      }
      window.history.back()
    },
    onError:(error: any)=>{
  ...
  const OnSubmit = handleSubmit(async (values, event) => {
    ...
    if(values.date===''){
      ...
      newHolidayEntry(holidayDetails)
    } catch (error: any) {
      setLoading(false)
      return error.statusText
    }
